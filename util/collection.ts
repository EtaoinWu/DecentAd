...
export function aoo2ooa<T extends string, U>(ooa: Record<T, U>[]): Record<T, U[]> {
...
export function ooa2aoo<T extends string, U>(ooa: Record<T, U[]>): Record<T, U>[] {
  const keys = Object.keys(ooa) as T[];
  return ooa[keys[0]].map((_, i) => Object.fromEntries(
